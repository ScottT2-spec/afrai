 ...
 * OpenAI-compatible API at api.groq.com — ultra-fast inference on LPUs.
 *
...
export class GroqProvider extends BaseProvider {
  ...
  async complete(request: CompletionRequest): Promise<CompletionResponse> {
    ...
    if (!response.ok) {
      ...
      throw new ProviderError(`Groq API error ${response.status}: ${errorBody}`, this.id, response.status);
    }
  ...
  async *stream(request: CompletionRequest): AsyncIterable<StreamChunk> {
    ...
    if (!response.ok) {
      const errorBody = await response.text().catch(() => 'Unknown error');
      throw new ProviderError(`Groq streaming error ${response.status}: ${errorBody}`, this.id, response.status);
    }
